...
from ctl.nextcloud_calendar import CalendarError, discover, events
...
class CalDavTests(unittest.TestCase):
    ...
    def test_event_projection_omits_sensitive_fields_and_limits_results(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            xml = f'''<d:multistatus xmlns:d="DAV:" xmlns:c="urn:ietf:params:xml:ns:caldav"><d:response><d:propstat><d:prop><c:calendar-data><![CDATA[{ics}]]></c:calendar-data></d:prop></d:propstat></d:response></d:multistatus>'''.encode()
            response = httpx.Response(207, content=xml)
