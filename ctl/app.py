...
@app.get("/api/v1/mcp/servers/{server_id}/activity")
def mcp_activity(server_id: str, request: Request, before: int = 0, limit: int = 50) -> dict:
    if not identity(request)["writes_enabled"]:
        return JSONResponse({"ok": False, "error": "operator identity required"}, status_code=403)
    if not any(item.id == server_id for item in load_mcp_catalog(load_registry())):
        return JSONResponse({"ok": False, "error": "unknown MCP server"}, status_code=404)
    return {"ok": True, "server_id": server_id,
            "calls": McpActivity().history(server_id, before=before, limit=limit)}


@app.get("/api/v1/mcp/servers/{server_id}/jobs")
def mcp_server_jobs(server_id: str, request: Request, limit: int = 30) -> dict:
    if not identity(request)["writes_enabled"]:
        return JSONResponse({"ok": False, "error": "operator identity required"}, status_code=403)
    if not any(item.id == server_id for item in load_mcp_catalog(load_registry())):
        return JSONResponse({"ok": False, "error": "unknown MCP server"}, status_code=404)
...
@app.get("/api/v1/mcp/servers/{server_id}/updates")
def mcp_server_updates(server_id: str, request: Request) -> dict:
    if not identity(request)["writes_enabled"]:
        return JSONResponse({"ok": False, "error": "operator identity required"}, status_code=403)
    server = next((item for item in load_mcp_catalog(load_registry()) if item.id == server_id), None)
    if not server:
        return JSONResponse({"ok": False, "error": "unknown MCP server"}, status_code=404)
    return {"ok": True, "server_id": server_id, "current_version": server.revision,
            "reviewed_update": server.reviewed_update,
            "detail": "Only checked-in reviewed releases can be installed."}


@app.put("/api/v1/mcp/servers/{server_id}/tools/{tool_name}/permission")
async def put_mcp_tool_permission(server_id: str, tool_name: str, request: Request) -> dict:
    operator = identity(request)
    if not operator["writes_enabled"] or not _mutation_allowed(request):
        return JSONResponse({"ok": False, "error": "operator mutation verification failed"}, status_code=403)
    state = ControlState.runtime()
    runtime = state.mcp_server(server_id) if state else None
    if not runtime or not any(item.get("id") == tool_name for item in runtime.get("tool_snapshot", [])):
        return JSONResponse({"ok": False, "error": "tool is not in the verified tool list"}, status_code=404)
    try:
        payload = await request.json()
        permission = payload["permission"]
        selected_tool = next(item for item in runtime["tool_snapshot"] if item.get("id") == tool_name)
        if selected_tool.get("risk") != "read" and permission == "auto":
            raise ValueError("write tools must require approval or be disabled")
...
@app.post("/api/v1/mcp/servers/{server_id}/tools/{tool_name}/prepare")
async def prepare_mcp_tool(server_id: str, tool_name: str, request: Request) -> dict:
    operator = identity(request)
    if not operator["writes_enabled"] or not _mutation_allowed(request):
        return JSONResponse({"ok": False, "error": "operator mutation verification failed"}, status_code=403)
    try:
        payload = await request.json()
        return await run_in_threadpool(mcp_console.prepare, server_id, tool_name,
                                       payload.get("arguments"), operator["username"])
    except (ValueError, TypeError, AttributeError) as exc:
        return JSONResponse({"ok": False, "error": str(exc)}, status_code=422)


@app.post("/api/v1/mcp/servers/{server_id}/tools/{tool_name}/execute")
async def execute_mcp_tool(server_id: str, tool_name: str, request: Request) -> dict:
    operator = identity(request)
    if not operator["writes_enabled"] or not _mutation_allowed(request):
        return JSONResponse({"ok": False, "error": "operator mutation verification failed"}, status_code=403)
    try:
        ...
        return await run_in_threadpool(
            lambda: mcp_console.execute(server_id, tool_name, payload.get("arguments"),
                                        operator["username"],
...
async def _queue_mcp_action(server_id: str, action: str, request: Request) -> dict:
    ...
    if action == "update" and not server.reviewed_update:
        return JSONResponse({"ok": False, "error": "no reviewed MCP update is available"}, status_code=409)
    states = {item["id"]: item["state"] for item in _service_snapshot(request)["services"]}
    if action != "prepare" and states.get(server.service_id) not in {"ready", "running", "degraded"}:
    ...
    if action == "prepare":
        ...
        if not installation or not installation.get("installed_at"):
            return JSONResponse({"ok": False, "error": "install the application before preparing its MCP"}, status_code=409)
    store = JobStore.runtime()
    if store is None:
        return JSONResponse({"ok": False, "error": "runtime job store is not initialized"}, status_code=503)
    try:
...
@app.post("/api/v1/mcp/servers/{server_id}/prepare")
async def prepare_mcp(server_id: str, request: Request) -> dict:
    return await _queue_mcp_action(server_id, "prepare", request)
...
@app.post("/api/v1/mcp/servers/{server_id}/update")
async def update_mcp(server_id: str, request: Request) -> dict:
    return await _queue_mcp_action(server_id, "update", request)
