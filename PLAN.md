...
| 16 | Dashboard split | **Simple (stdlib, :8799) vs real (React/FastAPI, :8787) — locked.** Simple dashboard install card delivers: host deps + Node + venv + dashboard build + systemd + **Docker/networks + Tailscale (up+serve) + Caddy**. Authentik + Vaultwarden are **user apps installed from the real dashboard**, not infra. The moment `:8787` is up it is tailnet-reachable. |
