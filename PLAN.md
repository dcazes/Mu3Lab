...
| 16 | Dashboard split | **Simple (stdlib, :8799) vs real (React/FastAPI, :8787) — locked.** Simple dashboard install card delivers: host deps + Node + venv + pip + dashboard source check + dashboard build + root `.env` + systemd service + **Docker engine + Tailscale (up+serve) + Caddy**, ending with a **fresh-login checkpoint** (logout/in → `./check.sh` → Resume) after which shared networks + Caddy finish. Authentik + Vaultwarden are **user apps installed from the real dashboard**, not infra. |
