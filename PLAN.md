...
| 16 | Dashboard split | **Simple (stdlib, :8799) vs real (React/FastAPI, :8787) — locked.** Simple dashboard install card delivers: host deps + Node + venv + pip + dashboard source check + dashboard build + root `.env` + systemd service + **Docker/networks + Tailscale (up+serve) + Caddy**. The moment `:8787` answers health checks it is tailnet-served. Authentik + Vaultwarden are **user apps installed from the real dashboard**, not infra. |
