...
export interface McpServer { id: string; name: string; service_id: string; kind: string; transport: string; app_state: string; enabled: boolean; prepared?: boolean; state: 'live' | 'degraded' | 'authentication_required' | 'disabled' | 'prepared' | 'unavailable' | 'starting' | 'incompatible' | 'failed' | 'stopped'; error?: string | null; last_verified_at?: string; auth: { type: string; scopes: string[]; configured: boolean }; review?: { status: string; repository: string; revision: string; preferred: boolean; note?: string }; configuration?: ServiceConfigField[]; tools: Array<{ id: string; title: string; risk: string; enabled: boolean; permission?: string; parameters?: Record<string, unknown> }>; }
export interface McpRegistryResponse { ok: boolean; servers: McpServer[]; summary: Record<string, number>; policy: string; }
export interface ChatProvider { id: 'lobehub' | 'open-webui'; name: string; ready: boolean; url: string; authentication: string; detail: string; }
